...
class LightChangeColorOnCommand(ICommand):
    """ Comando concreto """
...
class RemoteController:
    ...
    def button_pressed(self, name) -> None:
        if name in self._buttons:
            self._buttons[name].execute()
            self._undos.append((name, 'execute'))

    def button_undo(self, name) -> None:
        if name in self._buttons:
...
if __name__ == '__main__':
    ...
    bedroom_light_blue = LightChangeColorOnCommand(bedroom_light, 'blue')
    bedroom_light_red = LightChangeColorOnCommand(bedroom_light, 'red')
